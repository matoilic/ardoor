...
void SLAnimation::init(SLVec3f* controls)
{
   ...
   if (_keyframes.size() > 1)
   {
      ...
      _curve = new SLCurveBezier(points, times, _keyframes.size(), controls);
      delete[] points;
      delete[] times;
   }
...
transform matrix of the passed shape node. The shapes matrix is updated in 
fixed order: Translation, rotation and scale. 
*/
void SLAnimation::animate(SLShape* shape, SLfloat elapsedTimeSec) 
{
   ...
   if (k1 && k2)
   {
      ...
      if (k1->position() != k2->position())
         shape->translate(_curve->evaluate(easingTime));
      ...
      {  
         ...
         {  SLfloat angleDEG;
            ...
            shape->rotate(frac * angleDEG, axis);
         }
         else // otherwise do quaternion interpolation
            shape->multiply(k1->orientation().lerp(k2->orientation(), frac).toMat4());
      }
      // Add scaling if the keyframe scalings are different
      if (k1->scaling() != k2->scaling())
         shape->scale((1-frac)*k1->scaling() + frac*k2->scaling());
      
      // increment or decrement time
      _currentTime += elapsedTimeSec * _direction;
   }
...
SLfloat SLAnimation::easing(SLfloat time)
{
   ...
   switch (_easing)
   {
      ...
      case inSine:      y =      sin(x*SL_PI*0.5 - SL_PI*0.5) + 1.0f; break;
      case outSine:     y =      sin(x*SL_PI*0.5);                    break;
      case inOutSine:   y = 0.5f*sin(x*SL_PI - SL_PI*0.5) + 0.5f;     break;
