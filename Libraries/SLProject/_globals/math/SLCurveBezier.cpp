//#############################################################################
//  File:      Globals/SLCurveBezier.cpp
...
{  
   ...
   {  
      ...
      renderPoints.push_back(wm * _points[_count-1]);

      // add inputs points
      for (SLuint i = 0; i < _count; ++i)
         renderPoints.push_back(wm * _points[i]);
      
      // add control points
      for (SLuint i = 0; i < numControlPoints; ++i)
         renderPoints.push_back(wm * _controls[i]);

      // add tangent points
      for (SLuint i = 0; i < numControlPoints; ++i)
      {  renderPoints.push_back(wm * _controls[i]);
         int iPoint = (SLint)((SLfloat)i/2.0f + 0.5f);
         renderPoints.push_back(wm * _points[iPoint]);
      }
      
      // Generate finally the OpenGL rendering buffer
      _bufP.generate(&renderPoints[0], renderPoints.size(), 3);
   }
