   ...
   {  glTexImage2D(GL_TEXTURE_2D, 
      ...
      if (_min_filter>=GL_NEAREST_MIPMAP_NEAREST)
		{  SLstring sVersion = _stateGL->glVersion();
         SLfloat  fVersion = atof(sVersion.c_str());
         SLbool   foundOpenGLES2 = (sVersion.find("OpenGL ES 2")!=string::npos);
  
         if (foundOpenGLES2 || fVersion >= 3.0)
...
SLuint SLGLTexture::closestPowerOf2(SLuint num)
{
   ...
   if (num-prevPow2 < nextPow2-num) return prevPow2; else return nextPow2;
}
