...
{  assert(width>0 && height>0);
   ...
   {  
      ...
      default: 
         cout << "SLImage::Allocate: Unknown format" << endl;
         exit(1);
   }
   ...
   if (!_data)
   {  cout << "SLImage::Allocate: Allocation failed" << endl;
      exit(1);
   }
}
//-----------------------------------------------------------------------------
...
{    
   ...
   cout << "SLImage.load: Unsupported image file type: " << ext.c_str() << endl;
   exit(1);
}
//-----------------------------------------------------------------------------
...
void SLImage::loadJPG(SLstring filename)
{
   ...
   {  cout << "SLGLTexture::loadJPEG: Failed to load image: " << filename.c_str() << endl;
      exit(1);
   }
   ...
   {  jpeg_destroy_decompress(&cinfo);
      fclose(fp);
      cout << "SLGLTexture::loadJPEG: JPEG loading error." << endl;
      exit(1);
   }
   ...
   if (!_data)
   {  fclose(fp);
      cout << "SLGLTexture::loadJPEG: Memory allocation failed!" << endl;
      exit(1);
   }
   ...
   {  case JCS_GRAYSCALE:  
      ...
      case JCS_RGB:        
         ...
         break; 
      case JCS_UNKNOWN: 
         cout << "JCS_UNKNOWN: invalid color space" << endl;
         exit(1); break;
      case JCS_YCbCr:   
         cout << "JCS_YCbCr invalid color space" << endl;
         exit(1); break;
      case JCS_CMYK:    
         cout << "JCS_CMYK invalid color space" << endl;
         exit(1); break;
      case JCS_YCCK:    
         cout << "JCS_YCCK invalid color space" << endl;
         exit(1); break;
      default:
...
void SLImage::loadPNG(SLstring filename)
{
   ...
   if (!fp)
   {  fclose(fp);
      cout << "SLGLTexture::loadPNG: Failed to load image: " << filename.c_str() << endl;
   }
   ...
   {  fclose(fp);
      cout << "SLGLTexture::loadPNG: Not a valid PNG file: " << filename.c_str() << endl;
   }
   ...
   {  case PNG_COLOR_TYPE_GRAY: 
      ...
      case PNG_COLOR_TYPE_RGB_ALPHA: 
         ...
         break;
      default: 
         cout << "Wrong PNG color type!" << endl;
         exit(1); 
         break;
   }
...
{  
   ...
   if ((fp = fopen(filename.c_str(), "rb")) == NULL)
   {  fclose(fp);
      cout << "SLImage::loadBMP: Failed to open image: " << filename.c_str() << endl;
   }

   // Read data into bitmap file header into BFH struct
   if (fread((char*)&BFH, sizeof(sBMP_FILEHEADER),1, fp) != 1)
   {  fclose(fp);
      cout << "SLImage::loadBMP: Failed to read file header" << endl;
      exit(1);
   }
   ...
   {  fclose(fp);
      cout << "SLImage::loadBMP: Failed to read info header: " << filename.c_str() << endl;
      exit(1);
   }
   
   if (BIH.biBitCount!=24)
   {  fclose(fp);
      cout << "SLImage::loadBMP: Only 24 Bit BMP allowed: " << filename.c_str() << endl;
      exit(1);
   }
   
   if (BIH.biHeight < 0)
   {  fclose(fp);
      cout << "SLImage::loadBMP: biHeight<0" << endl;
      exit(1);
   }
   ...
   if (!tmpData)
   {  fclose(fp);
      cout << "SLImage::loadBMP: Not enough memory!" << endl;
      exit(1);
   }
   ...
   {  cout << "SLImage::loadBMP: Not enough data in file!" << endl;
      exit(1);
   }
...
{  
   if (!_data || !_width || !_height)
      cout << "SLGLTexture::savePNG: No data to write for file:" << filename.c_str() << endl;
   ...
   {  row_ptrs[h] = (png_bytep)(_data+((_height-(h+1))*_width*_bytesPerPixel));
	...
	if (!fp)
   {  fclose(fp);
      cout << "SLGLTexture::savePNG: Failed to create file: " << filename.c_str() << endl;
      exit(1);
   }
	...
	if (!png_ptr)
	{  fclose(fp);
      cout << "SLGLTexture::savePNG: Failed to create write struct." << endl;
      exit(1);
   }
	...
	if (!info_ptr)
	{  fclose(fp);
      cout << "SLGLTexture::savePNG: png_create_info_struct failed" << endl;
      exit(1);
   }

	if (setjmp(png_jmpbuf(png_ptr)))
	{  fclose(fp);
      cout << "SLGLTexture::savePNG: Error during init_io" << endl;
      exit(1);
   }
	...
	if (setjmp(png_jmpbuf(png_ptr)))
   {  fclose(fp);
      cout << "SLGLTexture::savePNG: Error during writing header" << endl;
      exit(1);
   }
   ...
   {  
      ...
      default: 
         cout << "Wrong GL color format." << endl;
         exit(1);
   }
	...
	if (setjmp(png_jmpbuf(png_ptr)))
   {  fclose(fp);
      cout << "SLGLTexture::savePNG: Error during writing bytes" << endl;
      exit(1);
   }
   ...
   ///////////////////////////////////
	...
	if (setjmp(png_jmpbuf(png_ptr)))
   {  fclose(fp);
      cout << "SLGLTexture::savePNG: Error during end of write" << endl;
      exit(1);
   }
...
{  SLuint   addr;
   ...
   {  case GL_RGB:
      ...
      case GL_RG:
         ...
         break;
      default: 
         cout << "SLImage::getPixeli: Unknown format!" << endl;
         exit(1);      
   }
   color /= 255.0f;   
   return color;
   ...
   {  case GL_RGB:
      ...
      case GL_RG:
         ...
         *(addr  ) = (SLubyte)(color.a * 255.0f);
         break;
      default: 
         cout << "SLImage::setPixeli: Unknown format!" << endl;
         exit(1);
   }
}
//-----------------------------------------------------------------------------
...
{  
   ...
   {  dstData = new SLubyte[dstBytesPerImage];
      if (!dstData) 
      {  cout << "SLImage::resize: Out of memory." << endl;
         exit(1);
      }
   }
...
{  
   ...
   {  
      ...
      if (!pDst) 
      {  cout << "SLImage::flipY(): Out of memory." << endl;
         exit(1);
      }
